...
import { ref } from 'vue'
import {
  adminDashboard, adminServices, adminAddons, adminStaff, adminStaffDetail, adminStaffOptions, apiError,
  ...
  toggleAdminService, updateAdminService, updateAdminStaff, createAdminAddon, updateAdminAddon, toggleAdminAddon,
  type AdminDashboard, type AdminRoster, type AdminService, type AdminAddon, type AdminStaffDetail, type AdminStaffOptions, type AdminStaffSummary,
} from '../lib/api'
...
export const useAdminStore = defineStore('admin', () => {
  ...
  const addons = ref<AdminAddon[]>([]); const addonSummary = ref({ active: 0, inactive: 0 }); const addonMeta = ref(emptyMeta()); const addonSearch = ref(''); const addonStatus = ref('')
  ...
  const dashboardKey='admin:dashboard';const servicesKey=(page=1)=>`admin:services:${page}:${serviceSearch.value.trim()}:${serviceStatus.value}`;const addonsKey=(page=1)=>`admin:addons:${page}:${addonSearch.value.trim()}:${addonStatus.value}`;const staffKey=(page=1)=>`admin:staff:${page}:${staffSearch.value.trim()}:${staffStatus.value}:${staffBookable.value}`
  ...
  async function loadAddons(page = 1,force=false): Promise<void> { await runCached(addonsKey(page),OPERATIONAL_TTL_MS,async () => { const response = await adminAddons({ page, q: addonSearch.value.trim() || undefined, status: addonStatus.value || undefined }); addons.value = response.data; addonSummary.value = response.summary; addonMeta.value = response.meta },force) }
  async function saveAddon(payload: Record<string, unknown>, id?: number): Promise<boolean> { return mutate(() => id ? updateAdminAddon(id, payload) : createAdminAddon(payload), async () => { invalidateMobileData('admin:'); await loadAddons(id ? addonMeta.value.current_page : 1,true) }) }
  async function toggleAddon(id: number): Promise<boolean> { return mutate(() => toggleAdminAddon(id), async () => { invalidateMobileData('admin:'); await loadAddons(addonMeta.value.current_page,true) }) }
  ...
  function reset():void{dashboard.value=null;services.value=[];serviceSummary.value={active:0,inactive:0};serviceMeta.value=emptyMeta();addons.value=[];addonSummary.value={active:0,inactive:0};addonMeta.value=emptyMeta();addonSearch.value='';addonStatus.value='';staff.value=[];staffSummary.value={active:0,inactive:0,bookable:0};staffMeta.value=emptyMeta();staffOptions.value=null;selectedStaff.value=null;roster.value=null;rosterWeek.value='';serviceSearch.value='';serviceStatus.value='';staffSearch.value='';staffStatus.value='';staffBookable.value='';clear()}

  return { dashboard, services, serviceSummary, serviceMeta, serviceSearch, serviceStatus, addons, addonSummary, addonMeta, addonSearch, addonStatus, staff, staffSummary, staffMeta, staffSearch, staffStatus, staffBookable, staffOptions, selectedStaff, roster, rosterWeek, loading, refreshing, working, error, notice, fields, hasDashboard, hasServices, hasStaff, loadDashboard, loadServices, saveService, toggleService, loadAddons, saveAddon, toggleAddon, loadStaff, loadStaffOptions, openStaff, saveStaff, addWeeklySchedule, removeWeeklySchedule, addException, removeException, loadRoster, copyRoster, addRosterShift, removeRosterShift, publishRoster, clear, reset }
})
