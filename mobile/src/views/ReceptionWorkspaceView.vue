...
<template><MobileWorkspaceShell :account-label="auth.user?.name ?? 'Reception'" navigation-label="Receptionist navigation" :active-id="tab" :items="items" :working="auth.working" :error="auth.error" @select="tab=$event as typeof tab" @sign-out="leave"><ReceptionDashboardView v-if="tab==='dashboard'" @navigate="tab=$event"/><ReceptionAppointmentsView v-else-if="tab==='appointments'"/><ReceptionCustomersView v-else-if="tab==='customers'"/><ReceptionPaymentsView v-else/></MobileWorkspaceShell></template>
