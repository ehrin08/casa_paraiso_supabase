...
export interface AttendanceScan { id: number; status: string; scanned_at: string | null; expires_at: string | null; therapist: AppointmentParty | null }
...
export async function rejectAttendanceScan(id: number): Promise<{ message: string }> { return (await getClient().post(`/attendance-station/scans/${id}/reject`)).data }
