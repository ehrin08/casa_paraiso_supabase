...
export interface StaffFeedback { id: number; rating: number; comment: string | null; sentiment: 'positive' | 'neutral' | 'negative'; analysis_version?: string | null; source?: 'rules' | 'model'; confidence?: string | null; evidence?: Record<string, unknown> | null; topics?: Array<{ key: string; polarity: 'positive' | 'negative'; matched_terms: string[] }>; model_candidate?: { label: string; confidence: string | null; version: string } | null; review_state?: 'unreviewed' | 'needs_review' | 'reviewed'; submitted_at: string | null; customer: AppointmentParty | null; service: AppointmentParty | null; appointment: { id: number; appointment_number: string } | null }
export interface AdminFeedbackOverview { total: number; positive_rate: number; negative_rate: number; previous_total: number; attention: number; date_from: string; date_to: string; service_breakdown: Array<{ id: number | null; name: string | null; total: number; negative: number }>; topic_breakdown: Array<{ key: string; total: number; negative: number }>; model_health?: { model_runs: number; disagreements: number; reviewed: number; average_confidence: number } }
...
export interface AttendanceRecord { id: number; attendance_date: string; time_in_at: string | null; time_out_at: string | null; status: 'open' | 'closed'; therapist: AppointmentParty | null; events: Array<{ id: number; action: string; source: string; occurred_at: string | null; reason: string | null; recorded_by: string | null }> }
...
export async function adminFeedback(params: { page?: number; sentiment?: string; q?: string; date_from?: string; date_to?: string; service_id?: number; topic?: string; source?: string; review_state?: string; confidence_min?: number } = {}) { return (await getClient().get('/admin/feedback', { params })).data as { data: StaffFeedback[]; summary: { positive: number; neutral: number; negative: number }; overview: AdminFeedbackOverview; meta: AppointmentListResponse['meta'] } }
...
export async function staffAttendance(): Promise<{ attendance: AttendanceRecord | null }> { return (await getClient().get('/staff/attendance')).data.data }
...
export async function adminAttendance(params: Record<string, string | number | undefined> = {}) { return (await getClient().get('/admin/attendance', { params })).data as { data: AttendanceRecord[]; staff: AppointmentParty[]; meta: AppointmentListResponse['meta'] } }
export async function correctAdminAttendance(id: number, payload: { action: 'time_in' | 'time_out'; occurred_at: string; reason: string }): Promise<{ data: AttendanceRecord; message: string }> { return (await getClient().patch(`/admin/attendance/${id}/correct`, payload)).data }
