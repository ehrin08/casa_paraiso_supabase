...
- `/api/v1/meta` implements URL-only pairing with a stable server UUID binding, CORS restrictions, and rate limiting. Development builds accept the exact Quick Tunnel host or APK link; hosted releases validate their compiled HTTPS Render origin automatically. No pairing PIN or verification endpoint is required.
...
- `mobile/` is a bundled Vue 3/TypeScript/Tailwind/Pinia/Capacitor 8 Android project. A hosted release validates, saves, and uses `VITE_PRODUCTION_BACKEND_URL` at startup, replaces any old tunnel pairing state, hides manual server entry/deep links, stores the bearer token in Android Keystore-backed secure storage, and routes authenticated users to a role-specific workspace shell without a remote `server.url`.
...
- `scripts/mobile-demo.ps1` owns development Quick Tunnel start, rotation, environment hardening/restoration, metadata checks, temporary debug/demo-APK browser delivery, URL-only optional ADB pairing, and shutdown.
...
- Hosted release APKs accept only their compiled HTTPS Render origin and validate `/api/v1/meta` before saving it. Development/demo APKs retain exact HTTPS `*.trycloudflare.com` pairing.
