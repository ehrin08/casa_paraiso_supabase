...
- The customer APK workspace now lists the signed-in customer's appointments with status filters, fixed 15-record pagination, visit details, add-ons, reward and feedback state, and transactional pre-start cancellation through the existing Laravel workflow. It also books confirmed appointments from live month availability with eligible therapist preferences, paid add-ons, and owned RFM vouchers; the API rechecks capacity transactionally and returns stable error envelopes. The matching `/api/v1/customer` endpoints enforce customer role and record ownership.
...
- Pairing does not grant an authenticated application session; it only identifies the backend. Customer feedback/profile, the remaining role workspaces, mobile Google exchange, PostgreSQL/Supabase cutover, release signing, and physical-device acceptance remain later milestones.
