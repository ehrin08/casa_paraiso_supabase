...
- Current verification covers 254 Laravel tests and 1,841 assertions on each of MariaDB and PostgreSQL, local full-row migration validation and CRUD integrity, 19 mobile unit tests, five phone-sized Playwright role workflows with axe accessibility scans across every main tab, the mobile production build, Capacitor Android synchronization, Android release unit/lint gates, a universal release APK signed with Android v2/v3 schemes, and phone-sized Quick Tunnel acceptance for customer, receptionist, therapist, admin, and protected super-administrator workspaces. Dedicated Supabase cutover, live Google-provider acceptance, signing-key backup, and installation on the user's physical phone are still required.
