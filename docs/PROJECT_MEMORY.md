...
- The mobile foundation now includes URL-only pairing, Sanctum device-token authentication, and role workspace shells. `/api/v1/meta`, `POST /api/v1/auth/login`, `GET /api/v1/auth/me`, and `POST /api/v1/auth/logout` are implemented; metadata validation establishes server identity only, while the app keeps a 30-day mobile bearer token in native secure storage.
