...
| Mobile pairing, authentication, role workflows, tunnel, or Android shell | `MOBILE_SUPABASE_PLAN.md`, `DOCKER_WORKFLOW.md` | `routes/api.php`, `MobilePairing`, mobile API controllers/resources, API middleware, `mobile/src`, `scripts/mobile-demo.ps1` | `MobilePairingApiTest`, `MobileAuthApiTest`, mobile customer, receptionist, therapist, and admin API tests, mobile unit tests, Android `assembleDebug` |
