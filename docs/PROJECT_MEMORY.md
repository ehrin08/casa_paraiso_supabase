...
- `mobile/e2e/role-workspaces.spec.ts` runs phone-sized Playwright smoke workflows for all five roles against deterministic API contracts and applies axe accessibility scans after every main navigation tab. Keep this suite alongside Vitest and live Quick Tunnel acceptance; it caught and now guards the payment-filter accessible-name requirement.
...
- `routes/api.php` owns the versioned mobile surface: pairing metadata/verification, Sanctum-protected mobile authentication/password change, customer appointment/booking/feedback/profile endpoints, receptionist dashboard/appointment/customer/transaction endpoints, and therapist dashboard/assigned-appointment/customer/transaction/feedback/commission endpoints.
...
- The Quick Tunnel pairing flow is implemented and live-verified. Mobile Google OAuth callback/exchange behavior is covered by backend and client tests, but the local environment has no Google client credentials, so live provider acceptance remains pending.
