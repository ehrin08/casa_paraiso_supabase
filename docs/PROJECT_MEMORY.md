...
- The customer mobile workspace now provides owned appointment history/details, status filtering, fixed pagination, add-on/reward/feedback state, pre-start cancellation, and transactional booking with eligible therapist preferences, paid add-ons, available RFM vouchers, and live month availability. `MobileCustomerAppointmentController`, `MobileCustomerBookingController`, `MobileAppointmentResource`, `mobile/src/stores/customerAppointments.ts`, `mobile/src/stores/customerBooking.ts`, `CustomerAppointmentsView.vue`, and `CustomerBookingView.vue` are the primary entry points.
...
- `routes/api.php` owns the versioned mobile surface: pairing metadata/verification, Sanctum-protected mobile authentication, and customer appointment history, availability, booking, detail, and cancellation endpoints.
...
| Mobile pairing, authentication, customer appointments, tunnel, or Android shell | `MOBILE_SUPABASE_PLAN.md`, `DOCKER_WORKFLOW.md` | `routes/api.php`, `MobilePairing`, `MobileAuthController`, `MobileCustomerAppointmentController`, `MobileCustomerBookingController`, `MobileAppointmentResource`, API middleware, `mobile/src`, `scripts/mobile-demo.ps1` | `MobilePairingApiTest`, `MobileAuthApiTest`, `MobileCustomerAppointmentApiTest`, `MobileCustomerBookingApiTest`, mobile unit tests, Android `assembleDebug` |
...
- Pairing, Sanctum device-token authentication, secure token storage, role workspace shells, and the customer appointment history/detail/cancellation/booking vertical slice are implemented. Customer feedback/profile, the remaining feature-complete role workspaces, mobile Google exchange, and release signing remain pending.
...
- The automated four-workspace smoke suite passes. Pairing, password sign-in, customer booking options, live availability, and slot selection were also verified through a phone-sized in-app browser against a Cloudflare Quick Tunnel on 2026-07-15.
