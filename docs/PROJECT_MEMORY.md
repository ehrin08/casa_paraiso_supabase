...
| Mobile pairing, authentication, customer/receptionist/therapist workflows, tunnel, or Android shell | `MOBILE_SUPABASE_PLAN.md`, `DOCKER_WORKFLOW.md` | `routes/api.php`, `MobilePairing`, mobile API controllers/resources, API middleware, `mobile/src`, `scripts/mobile-demo.ps1` | `MobilePairingApiTest`, `MobileAuthApiTest`, mobile customer, receptionist, and therapist API tests, mobile unit tests, Android `assembleDebug` |
...
- Pairing, Sanctum device-token authentication, secure token storage, role workspace shells, and the complete customer, receptionist, and therapist vertical slices are implemented. Admin and super-administrator feature-complete workspaces, mobile Google exchange, and release signing remain pending.
