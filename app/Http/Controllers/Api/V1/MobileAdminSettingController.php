...
class MobileAdminSettingController
{
    public function show(): JsonResponse
    {
        ...
        return response()->json(['data' => ['settings' => ['business_name' => $settings->business_name, 'contact_email' => $settings->contact_email, 'contact_phone' => $settings->contact_phone, 'business_address' => $settings->business_address, 'default_payment_method' => $settings->default_payment_method], 'payment_methods' => Transaction::PAYMENT_METHODS, 'operating' => ['timezone' => config('app.timezone'), 'opens_at' => config('casa.business_hours.opens_at'), 'closes_at' => config('casa.business_hours.closes_at'), 'slot_interval_minutes' => (int) config('casa.booking.slot_interval_minutes', 30), 'commission_rate' => number_format((float) config('casa.commissions.therapist_rate', 0.22), 4, '.', '')], 'security' => $this->securityChecks()]])->header('Cache-Control', 'no-store');
    }
