        ...
        <div class="space-y-3">
            @foreach($users as $managedUser)
                <form method="post" action="{{ route('admin.users.update', $managedUser) }}" class="casa-card grid gap-4 p-4 lg:grid-cols-[minmax(10rem,1fr)_minmax(13rem,1fr)_10rem_8rem_auto] lg:items-end">
                    ...
                    <div><x-input-label :for="'email-'.$managedUser->id" value="Google email"/><x-text-input :id="'email-'.$managedUser->id" name="email" type="email" class="mt-1" :value="$managedUser->email" :disabled="$managedUser->isSuperAdmin()" required/></div>
